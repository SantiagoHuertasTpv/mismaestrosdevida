        ...
        <div class="mb-20">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @forelse($capitulo->canciones as $cancion)
                    <div class="bg-white/5 backdrop-blur-md border border-white/10 p-8 rounded-3xl flex flex-col items-center hover:bg-white/10 transition-all shadow-2xl">
                        <h3 class="text-white font-black text-center mb-6 h-14 flex items-center justify-center uppercase tracking-tight leading-tight">
                            {{ $cancion->nombre }}
                        </h3>
                        
                        <div class="bg-white p-4 rounded-2xl shadow-white/10 shadow-lg">
                            {!! QrCode::size(160)->margin(1)->generate(url('/q/' . $cancion->slug)) !!}
                        </div>
                        
                        <p class="mt-6 text-amber-500 text-xs font-black uppercase tracking-[0.2em]">
                            Escaneo Directo
                        </p>
                    </div>
                @empty
                    <div class="col-span-full p-10 border border-dashed border-white/20 rounded-3xl text-center">
                        <p class="text-gray-500 italic">Este capítulo pronto tendrá su música disponible.</p>
                    </div>
                @endforelse
            </div>
        </div>
